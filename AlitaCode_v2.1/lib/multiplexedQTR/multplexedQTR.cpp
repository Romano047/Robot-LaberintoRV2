...
void multiplexedQTR::setTypeAnalog()
{
  _type = QTRType::Analog;
  _maxValue = 1023; // Arduino analogRead() returns a 10-bit value by default
}

void multiplexedQTR::setSensorPins(const uint8_t * pins, uint8_t sensorCount)
{
...
void multiplexedQTR::setTimeout(uint16_t timeout)
{
...
void multiplexedQTR::setSamplesPerSensor(uint8_t samples)
{
  if (samples > 64) { samples = 64; }
  _samplesPerSensor = samples;
}

void multiplexedQTR::setEmitterPin(uint8_t emitterPin)
{
  releaseEmitterPins();
...
void multiplexedQTR::setEmitterPins(uint8_t oddEmitterPin, uint8_t evenEmitterPin)
{
  releaseEmitterPins();
...
void multiplexedQTR::releaseEmitterPins()
{
...
void multiplexedQTR::setDimmingLevel(uint8_t dimmingLevel)
{
...
void multiplexedQTR::emittersOff(QTREmitters emitters, bool wait)
{
...
void multiplexedQTR::emittersOn(QTREmitters emitters, bool wait)
{
  bool pinChanged = false;
...
uint16_t multiplexedQTR::emittersOnWithPin(uint8_t pin)
{
...
void multiplexedQTR::emittersSelect(QTREmitters emitters)
{
...
void multiplexedQTR::resetCalibration()
{
...
void multiplexedQTR::calibrate(QTRReadMode mode)
{
...
void multiplexedQTR::calibrateOnOrOff(CalibrationData & calibration, QTRReadMode mode)
{
...
void multiplexedQTR::read(uint16_t * sensorValues, QTRReadMode mode)
{
  switch (mode)
  {
...
void multiplexedQTR::readCalibrated(uint16_t * sensorValues, QTRReadMode mode)
{
  // manual emitter control is not supported
  if (mode == QTRReadMode::Manual) { return; }
...
void multiplexedQTR::readPrivate(uint16_t * sensorValues, uint8_t start, uint8_t step)
{
...
uint16_t multiplexedQTR::readLinePrivate(uint16_t * sensorValues, QTRReadMode mode,
                         bool invertReadings)
{
  bool onLine = false;
